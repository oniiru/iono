...
function et_post_meta($callback_args) {
	...
	$et_post_type = isset( $temp_array['et_post_type'] ) ? $temp_array['et_post_type'] : 1;
	...
	<div style="margin: 13px 0 26px 4px;">
		...
		<input type="text" style="width: 30em;" value="<?php echo $et_custom_post_video; ?>" id="et_custom_post_video" name="et_custom_post_video" size="67" />
		<br />
...
function save_details($post_id){
	...
	$temp_array['et_post_type'] = isset($_POST["et_post_type"]) ? $_POST["et_post_type"] : 1;
	update_post_meta( $post->ID, "et_envisioned_settings", $temp_array );
	
	$et_custom_post_video = isset($_POST["et_custom_post_video"]) ? $_POST["et_custom_post_video"] : '';
	update_post_meta( $post->ID, "et_videolink", $et_custom_post_video );
	
	$et_custom_portrait_detection = isset( $_POST["et_custom_portrait_detection"] ) ? 1 : 0;
	update_post_meta( $post->ID, "et_image_detection", $et_custom_portrait_detection );
	
	$et_custom_image_type = isset($_POST["et_custom_image_type"]) ? $_POST["et_custom_image_type"] : 'l';
	update_post_meta( $post->ID, "et_imagetype", $et_custom_image_type );
}
?>