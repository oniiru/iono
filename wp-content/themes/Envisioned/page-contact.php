...
<?php 
	...
	if ( isset($_POST['et_contactform_submit']) ) {
		if ( !isset($_POST['et_contact_captcha']) || empty($_POST['et_contact_captcha']) ) {
			$et_error_message .= '<p>' . __('Make sure you entered the captcha. ','Envisioned') . '</p>';
			...
			$et_error_message .= '<p>' . __('You entered the wrong number in captcha. ','Envisioned') . $et_numbers_string . '</p>';
			...
			$et_error_message .= '<p>' . __('Make sure you fill all fields. ','Envisioned') . '</p>';
			$et_contact_error = true;
		}
		
		if ( !is_email( $_POST['et_contact_email'] ) ) {
			$et_error_message .= '<p>' . __('Invalid Email. ','Envisioned') . '</p>';
			$et_contact_error = true;
		}
	} else {
		$et_contact_error = true;
	...
	if ( !$et_contact_error ) {
		...
		$et_error_message = '<p>' . __('Thanks for contacting us','Envisioned') . '</p>';
	}
?>
					...
					<?php if ( $et_contact_error ) { ?>
						<form action="<?php echo(get_permalink($post->ID)); ?>" method="post" id="et_contact_form">
							<div id="et_contact_left">
								<p class="clearfix">
									<input type="text" name="et_contact_name" value="<?php if ( isset($_POST['et_contact_name']) ) echo esc_attr($_POST['et_contact_name']); else _e('Name','Envisioned'); ?>" id="et_contact_name" class="input" />
								</p>
								
								<p class="clearfix">
									<input type="text" name="et_contact_email" value="<?php if ( isset($_POST['et_contact_email']) ) echo esc_attr($_POST['et_contact_email']); else _e('Email Address','Envisioned'); ?>" id="et_contact_email" class="input" />
								</p>
								
								<p class="clearfix">
									<input type="text" name="et_contact_subject" value="<?php if ( isset($_POST['et_contact_subject']) ) echo esc_attr($_POST['et_contact_subject']); else _e('Subject','Envisioned'); ?>" id="et_contact_subject" class="input" />
								</p>
							</div> <!-- #et_contact_left -->
							...
							<p class="clearfix">
								<textarea class="input" id="et_contact_message" name="et_contact_message"><?php if ( isset($_POST['et_contact_message']) ) echo esc_attr($_POST['et_contact_message']); else _e('Message','Envisioned'); ?></textarea>
							</p>
								
							<input type="hidden" name="et_contactform_submit" value="et_contact_proccess" />
							
							<input type="reset" id="et_contact_reset" value="<?php _e('Reset','Envisioned'); ?>" />
							<input class="et_contact_submit" type="submit" value="<?php _e('Submit','Envisioned'); ?>" id="et_contact_submit" />
						</form>
					<?php } ?>
				</div> <!-- end #et-contact -->
