...
function pmpromc_user_register($user_id)
{
	...
	if(!empty($options['users_lists']) && !empty($options['api_key']))
	{
		...
		$firstName = get_user_meta( $list_user->ID, 'first_name', true);
		$lastName = get_user_meta( $list_user->ID, 'last_name', true);
		...
		{					
			//subscribe them
		$api->listSubscribe($list, $list_user->user_email, $fullName, "html", false);
		}
	}
}
