...
function pmproa_process_affiliate_transaction($morder) {
	//Get email of a user associated with current order
	global $wpdb;
	$email = $wpdb->get_var('SELECT user_email FROM ' . $wpdb->users . ' WHERE ID=' . intval($morder->user_id));
	//Get refferer ID from database
	$referrer = pmproa_get_referrer($morder->user_id);
	if($referrer) {
		//If referrer ID is present, generate a sale
		do_action('wp_affiliate_process_cart_commission', array("referrer" => $referrer, "sale_amt" => $morder->subtotal, "txn_id" => $morder->code, "buyer_email" => $email));
	}
}
...
function pmproa_init(){
	...
	if(pmproa_dependencies_check()) {
		...
		add_action('wp_affiliate_process_cart_commission', 'pmproa_process_affiliate_transaction');		
	}
}
